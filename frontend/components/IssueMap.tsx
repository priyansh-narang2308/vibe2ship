    ...
    <div className="relative h-[420px] w-full rounded-3xl border border-indigo-100 bg-linear-to-b from-slate-50 to-indigo-50/20 overflow-hidden shadow-2xs">
      ...
      >
        ...
        <circle
          ...
          cy="50%"
          r="100"
          fill="oklch(0.52 0.22 265 / 2%)"
          stroke="oklch(0.52 0.22 265 / 15%)"
          strokeWidth="1.5"
        />
        <circle
          cx="50%"
      ...
      >
        <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-indigo-500/10">
          ...
          <span
            className="absolute -inset-1 animate-ping rounded-full border border-indigo-400/40 opacity-70"
      ...
      {issues.map((issue) => {
        ...
        return (
          ...
          >
            <div
              className={`relative flex h-9 w-9 items-center justify-center rounded-full border-3 bg-white shadow-md transition-all duration-300 ${
                isSelected
                  ? "scale-125 border-indigo-500 shadow-glow"
                  : "border-slate-100 hover:scale-115"
              }`}
            >
              <MapPin
                className={`h-4.5 w-4.5 transition-transform duration-200 ${styleInfo.color} ${isSelected ? "animate-bounce" : ""}`}
              />
              <span
                className={`absolute -inset-0.5 rounded-full border-2 border-transparent animate-ping ${isSelected ? "border-indigo-400/30" : ""}`}
              />
            </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`inline-flex items-center rounded-lg px-2 py-0.5 text-[8px] font-extrabold border ${
